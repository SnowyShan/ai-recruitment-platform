...
def search_question_bank(
    ...
    if job_id is not None:
        setup_row = conn.execute(
            "SELECT question_ids FROM job_setup WHERE job_id=?", (job_id,)
        ).fetchone()
...
class JobSetupRequest(BaseModel):
    ...
    generate_video: bool = (
        False  # Off by default — only generate D-ID videos when explicitly requested
    )
...
def setup_job(job_id: int, req: JobSetupRequest, background_tasks: BackgroundTasks):
    ...
    existing = conn.execute(
        "SELECT job_id FROM job_setup WHERE job_id=?", (job_id,)
    ).fetchone()
...
def get_job_setup_status(job_id: int):
    ...
    return {
        ...
        "domain": row["domain"],
        "video_interview_enabled": bool(row["video_interview_enabled"])
        if "video_interview_enabled" in row.keys()
        else False,
    }
...
def generate_videos_for_job(job_id: int, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(
        _generate_videos_bg, job_id=job_id, question_ids=question_ids
    )
    return {
...
def _get_video_provider_setting() -> str:
    ...
    try:
        conn = get_conn()
        row = conn.execute(
            "SELECT value FROM settings WHERE key='video_provider'"
        ).fetchone()
        conn.close()
...
def _generate_videos_bg(job_id: int, question_ids: list):
    ...
    print(
        f"[VIDEO GEN] Job {job_id}: generating videos for {len(question_ids)} questions via {provider_name}"
    )
    video_provider = vc.get_provider(provider_name)
    for q_id in question_ids:
        conn = get_conn()
        row = conn.execute(
            "SELECT question, voice_text, video_path, audio_path FROM questions WHERE id=?",
            (q_id,),
        ).fetchone()
        conn.close()
        if not row:
        ...
        audio_path = (
            os.path.join(AUDIO_DIR, row["audio_path"]) if row["audio_path"] else None
        )
        try:
            ...
            if video_result:
                ...
                conn = get_conn()
                conn.execute(
                    "UPDATE questions SET video_path=? WHERE id=?",
                    (video_filename, q_id),
                )
                conn.commit()
                conn.close()
...
def _run_job_setup(job_id: int, req: JobSetupRequest):
    ...
    try:
        ...
        if needed > 0:
            ...
            for q in generated:
                ...
                if req.generate_video:
                    ...
                    video_filename_stored = (
                        os.path.basename(video_result) if video_result else None
                    )
                ...
                conn.execute(
                    ...
                    (
                        ...
                        q.get("voice_text", ""),
                        q.get("topic", ""),
                        audio_filename if tts_ok else None,
                        video_filename_stored,
        ...
        conn.close()
        print(
            f"[JOB SETUP] Job {job_id} ready — {len(selected_ids)} technical questions"
        )
        ...
        conn = get_conn()
        conn.execute(
            "UPDATE job_setup SET status='failed', updated_at=? WHERE job_id=?",
...
def probe_assess(req: ProbeAssessRequest):
    result = ai.assess_answer_depth(
        req.question, req.answer, req.job_description, req.seniority_bar
    )
    return result
...
def toggle_core_competency(question_id: str, req: CoreCompetencyToggle):
    ...
    if req.enabled:
        probes = ai.generate_probes(
            row["question"], row["topic"] or "", req.job_description
        )
...
def evaluate_answer(req: EvaluateRequest):
    result = ai.evaluate_answer(
        ...
        req.job_description,
        req.seniority_bar,
        req.hardcoded_acceptable_answer,
    )
    return result
...
async def transcribe_audio(file: UploadFile = File(...)):
    ...
    if len(audio_bytes) > MAX_AUDIO_SIZE:
        raise HTTPException(
            status_code=400,
            detail=f"Audio file too large. Maximum size is 25MB, got {len(audio_bytes) / (1024 * 1024):.1f}MB",
        )
    ...
    try:
        ...
        @retry(
            ...
            retry=retry_if_exception_type((TimeoutException, Exception)),
            reraise=True,
        )
        def _transcribe():
            with open(tmp_path, "rb") as f:
                return client.audio.transcriptions.create(
                    model="whisper-1", file=f, timeout=30.0
                )
...
def create_session(req: SessionCreate):
    ...
    conn = get_conn()
    setup = conn.execute(
        "SELECT * FROM job_setup WHERE job_id=?", (req.job_id,)
    ).fetchone()
    conn.close()
    ...
    video_interview_enabled = (
        bool(setup["video_interview_enabled"])
        if "video_interview_enabled" in setup.keys()
        else False
    )
    ...
    for q_id in question_ids[:num_technical]:
        ...
        if row:
            q = dict(row)
            q["type"] = "technical"
            q["is_core_competency"] = (
                bool(row["is_core_competency"])
                if "is_core_competency" in row.keys()
                else False
            )
            probes = (
                json.loads(row["probe_questions"]) if row["probe_questions"] else []
            )
            ...
            q["audio_url"] = (
                f"/audio/{q['audio_path']}" if q.get("audio_path") else None
            )
            q["video_url"] = (
                f"/video/{q['video_path']}" if q.get("video_path") else None
            )
    ...
    for i, q in enumerate(behavioral_qs):
        ...
        audio_path = os.path.join(AUDIO_DIR, audio_filename)
        tts_ok = ai.generate_and_store_tts(
            q.get("voice_text") or q["question"], audio_path
        )
    ...
    if req.verify_coding_ability:
        ...
        tts_ok = ai.generate_and_store_tts(
            coding_q.get("voice_text") or coding_q["question"], coding_audio_path
        )
    ...
    conn.execute(
        ...
        (
            ...
            "{}",
            "active",
            1 if req.verify_coding_ability else 0,
            1 if video_interview_enabled else 0,
            datetime.utcnow().isoformat(),
        ),
    )
    conn.commit()
...
def get_session(session_id: str):
    ...
    return {
        ...
        "verify_coding_ability": bool(row["verify_coding_ability"])
        if "verify_coding_ability" in row.keys()
        else False,
        "video_interview_enabled": bool(row["video_interview_enabled"])
        if "video_interview_enabled" in row.keys()
        else False,
        "draw_answers": json.loads(row["draw_answers"])
        if "draw_answers" in row.keys() and row["draw_answers"]
        else None,
        "proctoring_data": json.loads(row["proctoring_data"])
        if "proctoring_data" in row.keys() and row["proctoring_data"]
        else None,
    }
...
def submit_answer(session_id: str, req: AnswerSubmit):
    ...
    evaluation = ai.evaluate_answer(
        q["question"], req.answer, row["job_description"], row["seniority_bar"], None
    )
...
class CompleteRequest(BaseModel):
    ...
    proctoring_data: Optional[dict] = (
        None  # {events: [...], snapshots: [...], identity_photo: str|null}
    )
...
def complete_session(session_id: str, req: CompleteRequest = CompleteRequest()):
    ...
    settings_row = conn.execute(
        "SELECT value FROM settings WHERE key = 'evaluation_prompt'"
    ).fetchone()
    ...
    conn.execute(
        ...
        (
            json.dumps(report),
            json.dumps(draw_answers) if draw_answers else None,
...
def get_testdata(folder: str):
    ...
    if not os.path.isdir(folder_path):
        raise HTTPException(
            status_code=404, detail=f"Test data folder '{folder}' not found"
        )
    ...
    if not os.path.exists(jd_path) or not os.path.exists(resume_path):
        raise HTTPException(
            status_code=404, detail=f"Missing jd.txt or resume.txt in {folder}"
        )
    return {
        "folder": folder,
...
def get_settings():
    ...
    video_provider = _get_setting(
        conn, "video_provider", os.getenv("VIDEO_PROVIDER", "did")
    )
    conn.close()
    return SettingsResponse(
        evaluation_prompt=evaluation_prompt, video_provider=video_provider
    )
...
def update_settings(req: SettingsUpdate):
    ...
    if req.video_provider is not None:
        ...
        if req.video_provider not in allowed:
            conn.close()
            raise HTTPException(
                status_code=400, detail=f"video_provider must be one of: {allowed}"
            )
